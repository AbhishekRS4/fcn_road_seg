...
class FCN:
    ...
    def fcn_8(self):
        self.conv6 = self._get_conv2d_layer(self.pool5, 512, [7, 7], [1, 1],
            self.padding, self.data_format, name="conv6")
        self.elu6 = self._get_elu_activation(self.conv6, name="elu6")

        self.conv7 = self._get_conv2d_layer(self.elu6, 128, [1, 1], [1, 1],
            self.padding, self.data_format, name="conv7")
        self.elu7 = self._get_elu_activation(self.conv7, name="elu7")

        self.conv8 = self._get_conv2d_layer(self.elu7, self.num_classes, [1, 1],
            [1, 1], self.padding, self.data_format, name="conv8")
        self.elu8 = self._get_elu_activation(self.conv8, name="elu8")

        self.conv_tr1 = self._get_conv2d_transpose_layer(self.elu8, self.num_classes,
            [4, 4], [2, 2], self.padding, self.data_format, name="conv_tr1")
        self.conv9 = self._get_conv2d_layer(self.pool4, self.num_classes, [1, 1],
            [1, 1], self.padding, self.data_format, name="conv9")
        self.fuse1 = tf.add(self.conv_tr1, self.conv9, name="fuse1")

        self.conv_tr2 = self._get_conv2d_transpose_layer(self.fuse1, self.num_classes,
            [4, 4], [2, 2], self.padding, self.data_format, name="conv_tr2")
        self.conv10 = self._get_conv2d_layer(self.pool3, self.num_classes, [1, 1],
            [1, 1], self.padding, self.data_format, name="conv10")
        self.fuse2 = tf.add(self.conv_tr2, self.conv10, name="fuse2")

        self.conv_tr3 = self._get_conv2d_transpose_layer(self.fuse2, self.num_classes,
            [16, 16], [8, 8], self.padding, self.data_format, name="conv_tr3")
        self.logits = self._get_conv2d_layer(self.conv_tr3, self.num_classes, [1, 1],
            [1, 1], self.padding, self.data_format, name="logits")
    ...
    def fcn_16(self):
        ...
        self.conv_tr2 = self._get_conv2d_transpose_layer(self.fuse1, self.num_classes,
            [32, 32], [16, 16], self.padding, self.data_format, name="conv_tr2")
        self.logits = self._get_conv2d_layer(self.conv_tr2, self.num_classes,
            [1, 1], [1, 1], self.padding, self.data_format, name="logits")
    ...
    def fcn_32(self):
        ...
        self.conv_tr1 = self._get_conv2d_transpose_layer(self.elu8, self.num_classes,
            [64, 64], [32, 32], self.padding, self.data_format, name="conv_tr1")
        self.logits = self._get_conv2d_layer(self.conv_tr1, self.num_classes,
            [1, 1], [1, 1], self.padding, self.data_format, name="logits")
    ...
    def _get_conv2d_layer(self, input_tensor, num_filters, kernel_size, strides, padding, data_format, name="conv"):
        conv_2d_layer = tf.layers.conv2d(inputs=input_tensor, filters=num_filters, kernel_size=kernel_size,
            strides=strides, padding=padding, data_format=data_format, name=name
        )
    ...
    def _get_conv2d_transpose_layer(self, input_tensor, num_filters, kernel_size, strides, padding, data_format, name="conv_tr"):
        conv_2d_tr_layer = tf.layers.conv2d_transpose(inputs=input_tensor, filters=num_filters,
            kernel_size=kernel_size, strides=strides, padding=padding, data_format=data_format, name=name
        )
    ...
    def _max_pool(self, input_layer, name):
        max_pool_layer = tf.nn.max_pool(value=input_layer, ksize=self.pool_kernel, strides=self.pool_strides,
            padding=self.padding, data_format=self.vgg_data_format, name=name
        )
    ...
    def _conv_layer(self, input_layer, name):
        with tf.variable_scope(name):
            ...
            conv = tf.nn.conv2d(input=input_layer, filter=conv_kernel, strides=self.conv_strides,
                padding=self.padding, data_format=self.vgg_data_format, name=name
            )
